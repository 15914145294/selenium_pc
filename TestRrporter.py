...
class TestReport(object):
	...
	def WriteHTML(self, testcaseinfo):
		...
		if testcaseinfo.result == "Failed":
			mytablerow = "<tr><td>{0}</td><td>{1}</td><td>{2}</td><td " \
			             "bgcolor=\"#FF0000\">{3}</td><td>{4}</td><td>{5}" \
			             "</td><td>{6}</td><td>{7}</td></tr>". \
				format(testcaseinfo.id, testcaseinfo.name, testcaseinfo.owner,
			           testcaseinfo.result, testcaseinfo.starttime, testcaseinfo.endtime,
			           testcaseinfo.secondsDuration, testcaseinfo.errorinfo)
		else:
			mytablerow = "<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}" \
			             "</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td></tr>".\
				format(
				testcaseinfo.id, testcaseinfo.name, testcaseinfo.owner, testcaseinfo.result,
				testcaseinfo.starttime,
