...
class Pretreat_img():
    ...
    def pretreat_image(self,image):
        ...
        global pretreat
        pretreat=Pretreat_img()

        # 二值化,得到0/255二值图片
        # 阀值threshold = 180
        image = pretreat.iamge2imbw(image,180)

        # 对二值图片进行降噪
        # N = 4
        pretreat.clear_noise(image,4)

        # 去除外边框
        # 原图大小:122*54
        # 左上右下,左 <= x < 右
        box = (   8,   10,  150,  60 )  # 8,   10,  118,  50
    ...
    def clear_noise(self,image,N):
        ...
        for x in range(1,Width - 1):
            for y in range(1,Height - 1):
                color = pretreat.get_near_pixel(image,x,y,N)
                if color != None:
...
if __name__ == '__main__':
    pretreat=Pretreat_img()
    image = Image.open('E://aa.jpg')
    image1 = pretreat.pretreat_image(image)
    image1.save('E://image1.jpg')
    print image1
    image1.show()